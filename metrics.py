...
def accuracy_thresh(
    ...
    **kwargs
    ...
    for i in range(len(y_pred)):
        out_label = np.where((y_pred[i,:] == 1).cpu())[0].tolist()
        real_label = np.where((y_true[i,:] == 1).cpu())[0].tolist()
        print(out_label,real_label)
        count = 0.
        if out_label == []:
            count += 1
        if set(out_label).issubset(set(real_label)) and out_label != []:
            acc += 1
    acc = acc / len(y_pred)
    count = count / len(y_pred)
    # print(f'the proportion of empty output is {count}')
...
def metrics_expert(y_pred, y_true):
    with open('/home/pg-task11/project/data_multi/val_data.csv', 'r') as csvfile:
        reader = csv.reader(csvfile)
        classes = list(reader)[0][1:]
    ...
    recalls = []
    for i in range(10):
        precision = sum(add[:,i]==2) / sum(pred[:,i]==1)
        recall = sum(add[:,i]==2) / sum(target[:,i]==1)
        precisions.append(precision.item())
        recalls.append(recall.item())
    return precisions,recalls

def metrics_random(y_pred, y_true):
    with open('/home/pg-task11/project/data_multi/val_data.csv', 'r') as csvfile:
        reader = csv.reader(csvfile)
        classes = list(reader)[0][1:]
    target = np.array(y_true)
